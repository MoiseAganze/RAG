...
<aside class="sidebar" id="sidebar">
  ...
  <div class="conv-section" id="conv-section">
    ...
    <?php else: ?>
      <?php foreach ($grouped as $label => $items): ?>
        <div class="conv-group-label"><?= htmlspecialchars($label) ?></div>
        <?php foreach ($items as $c): ?>
          ...
          <div
            ...
            onclick='selectConv(<?= $c['id'] ?>, <?= htmlspecialchars(json_encode($c['title']), ENT_QUOTES, 'UTF-8') ?>)'
          >
            <span class="conv-title"><?= htmlspecialchars($c['title']) ?></span>
          </div>
        <?php endforeach; ?>
      <?php endforeach; ?>
    <?php endif; ?>
  ...
  <nav class="sidebar-nav">
    <a href="chat.php" class="sidebar-nav-link <?= $currentPage === 'chat' ? 'active' : '' ?>">
    ...
    <a href="indexation.php" class="sidebar-nav-link <?= $currentPage === 'indexation' ? 'active' : '' ?>">
    ...
    <a href="admins.php" class="sidebar-nav-link <?= $currentPage === 'admins' ? 'active' : '' ?>">
      <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>
      Gestion des admins
    </a>
    <?php endif; ?>

    <a href="settings.php" class="sidebar-nav-link <?= $currentPage === 'settings' ? 'active' : '' ?>">
    ...
    <div class="sidebar-user">
      <div class="user-avatar"><?= strtoupper(mb_substr($user['prenom'], 0, 1)) ?></div>
      <div class="user-info">
        <div class="user-name"><?= htmlspecialchars($user['prenom'] . ' ' . $user['nom']) ?></div>
        <div class="user-role"><?= $user['role'] === 'admin_full' ? 'Administrateur' : 'Utilisateur' ?></div>
      </div>
